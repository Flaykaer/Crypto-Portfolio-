...
try:
    with open('/projects/Ver31/stats/alerts_stat.txt', 'r') as f:
        last = f.read()
...
try:
    with open('/projects/Ver31/stats/day_check.txt', 'r') as f:
        last = f.read()
...
try:
    with open('/projects/Ver31/stats_4h/alerts_stat_4h.txt', 'r') as f:
        last = f.read()
...
try:
    with open('/projects/Ver31/stats_4h/alerts_stat_1h.txt', 'r') as f:
        last = f.read()
...
try:
    with open('/projects/Ver31/stats_4h/hour_check_1h.txt', 'r') as f:
        last = f.read()
...
try:
    with open('/projects/Ver31/channel_price.txt', 'r') as f:
        last = f.read()
        channel_price = json.loads(last)

    with open('/projects/Ver31/channel_time.txt', 'r') as f:
        last = f.read()
        channel_time = json.loads(last)

    with open('/projects/Ver31/alerts_MT.txt', 'r') as f:
        last = f.read()
...
try:
    with open('/projects/Ver31/channel_cond.txt', 'r') as f:
        last = f.read()
...
try:
    with open('/projects/Ver31/count_trades.txt', 'r') as f:
        last = f.read()
...
try:
    ...
    with open('/projects/Ver31/lim_amp.txt', 'r') as f:
        last = f.read()
        lim_amp = json.loads(last)

    with open('/projects/Ver31/lim_atr.txt', 'r') as f:
        last = f.read()
        lim_atr = json.loads(last)

    '''
    with open('/projects/Ver31/f1.txt', 'r') as f:
        last = f.read()
        f1 = json.loads(last)

    with open('/projects/Ver31/f2.txt', 'r') as f:
        last = f.read()
        f2 = json.loads(last)
    '''

    with open('/projects/Ver31/current_minute_prices.txt', 'r') as f:
        last = f.read()
        current_minute_prices = json.loads(last)

    with open('/projects/Ver31/current_minute_start.txt', 'r') as f:
        last = f.read()
...
############
try:
    with open('/projects/Ver31/max_average_bar_price.txt', 'r') as f:
        last = f.read()
...
try:
    with open('/projects/Ver31/time_last_kline.txt', 'r') as f:
        last = f.read()
...
try:
    with open('/projects/Ver31/red_stop.txt', 'r') as f:
        last = f.read()
...
try:
    with open('/projects/Ver31/orange_stop.txt', 'r') as f:
        last = f.read()
...
try:
    with open('/projects/Ver31/yellow_stop.txt', 'r') as f:
        last = f.read()
...
try:
    with open('/projects/Ver31/last_atr.txt', 'r') as f:
        last = f.read()
...
try:
    with open('/projects/Ver31/orange_ten_min_start.txt', 'r') as f:
        last = f.read()
...
try:
    with open('/projects/Ver31/ten_min_start.txt', 'r') as f:
        last = f.read()
...
try:
    with open('/projects/Ver31/sl_time.txt', 'r') as f:
        last = f.read()
...
try:
    with open('/projects/Ver31/green_mile.txt', 'r') as f:
        last = f.read()
        green_mile = json.loads(last)

    with open('/projects/Ver31/green_mile_time.txt', 'r') as f:
        last = f.read()
        green_mile_time = json.loads(last)

    with open('/projects/Ver31/green_mile_price.txt', 'r') as f:
        last = f.read()
        green_mile_price = json.loads(last)

    with open('/projects/Ver31/on_w4.txt', 'r') as f:
        last = f.read()
        on_w4 = json.loads(last)

    with open('/projects/Ver31/on_w4_price.txt', 'r') as f:
        last = f.read()
        on_w4_price = json.loads(last)

    with open('/projects/Ver31/on_w4_time.txt', 'r') as f:
        last = f.read()
...
try:
    with open('/projects/Ver31/max_price.txt', 'r') as f:
        last = f.read()
        max_price = json.loads(last)

    with open('/projects/Ver31/stop.txt', 'r') as f:
        last = f.read()
...
try:
    with open('/projects/Ver31/rsl_time.txt', 'r') as f:
        last = f.read()
...
def ver_trade(channel_data_price, symb, price_last_alert_algo_1, time_last_alert_algo_1, time_tick, price, alerts,
                ...
                green_psar, k0, k1, price_acc, price_last_dinamic_sl, time_check_sl, check_volatility, q70, col_trades, number_of_trades, antifrod_2, last_10_avg_vol, zatupok): 
        
        def kline_MT_9(i,k):    
            if check_wtp[symb][i] == 0:
                if price_acc[symb] == []:
                    bot.send_message(error_tg, f'ver 31 price_acc[{symb}] == [] in kline_MT_9({i}, {k})')  ### отладка - поиск min([])   -nk-
...
#                         print(last_atr[symb])
            ...
            elif alerts_MT[symb][i] == 1 and len(channel_price[symb][i]) == 0 and price >= price_last_alert_algo_1[symb][i] * 1.005 and\
            antifrod_2[symb][i]:
                channel_price[symb][i] = channel_price[symb][i] + [price]
                channel_time[symb][i] = channel_time[symb][i] + [time_tick]
                alerts_MT[symb][i] = 2
    ...
    #                       last_atr[symb] / price * 100, "%")               ### Для отладки - nk
            ...
            # открытие сделки 
                    ...
                    (len(channel_price[symb][i]) == 0 or (len(channel_price[symb][i]) >= 2 and price > max(channel_price[symb][i]) * 1.005)) and\
...
# =============================================================================
            ...
            ################### Выходы 4w по стопам Y,O 22_3################################
            elif alerts_MT[symb][i] == 2 and on_w4[symb][i]:
                if all(pr < on_w4_price[symb][i] * 0.99 for pr in channel_data_price[symb][-20:]):
                    alerts_MT[symb][i] = 1
                    channel_price[symb][i] = channel_price[symb][i] + [price]
                    channel_time[symb][i] = channel_time[symb][i] + [time_tick]
                ...
                elif price > on_w4_price[symb][i] *1.02:
                    alerts_MT[symb][i] = 1
                    channel_price[symb][i] = channel_price[symb][i] + [price]
                    channel_time[symb][i] = channel_time[symb][i] + [time_tick]
                ...
                elif time_tick > on_w4_time[symb][i] + 8*60*1000 and price > np.median(price_acc[symb][-30:-5])*1.0017:
                    alerts_MT[symb][i] = 1
                    channel_price[symb][i] = channel_price[symb][i] + [price]
                    channel_time[symb][i] = channel_time[symb][i] + [time_tick]
                ...
                elif time_tick > on_w4_time[symb][i] + 3*60*1000 and price > np.mean(price_acc[symb][-10:-3])*1.005:
                    alerts_MT[symb][i] = 1
                    channel_price[symb][i] = channel_price[symb][i] + [price]
                    channel_time[symb][i] = channel_time[symb][i] + [time_tick]
            ...
            ################################## Зеленые горы 22_1############################################
            ...
            elif (alerts_MT[symb][i] == 2) and time_tick > time_last_alert_algo_1[symb][i] + 30 * 60 * 1000 and green_mile[symb][i]:
                if all(pr < green_mile_price[symb][i] * 0.99 for pr in channel_data_price[symb][-20:]):
                    alerts_MT[symb][i] = 1
                    channel_price[symb][i] = channel_price[symb][i] + [price]
                    channel_time[symb][i] = channel_time[symb][i] + [time_tick]
                ...
                elif price > green_mile_price[symb][i] *1.02:
                    alerts_MT[symb][i] = 1
                    channel_price[symb][i] = channel_price[symb][i] + [price]
                    channel_time[symb][i] = channel_time[symb][i] + [time_tick]
                ...
                elif time_tick > green_mile_time[symb][i] + 20*60*1000 and price > np.median(price_acc[symb][-30:-5])*1.0017 and time_tick>channel_time[symb][i][-1] + 20*60*1000:
                    alerts_MT[symb][i] = 1
                    channel_price[symb][i] = channel_price[symb][i] + [price]
                    channel_time[symb][i] = channel_time[symb][i] + [time_tick]
                ...
                elif time_tick > green_mile_time[symb][i] + 15*60*1000 and price > np.mean(price_acc[symb][-10:-3])*1.005 and time_tick>channel_time[symb][i][-1] + 12*60*1000:
                    alerts_MT[symb][i] = 1
                    channel_price[symb][i] = channel_price[symb][i] + [price]
                    channel_time[symb][i] = channel_time[symb][i] + [time_tick]
...
##################################################### Конец вставки 3 SL
        ...
        if alerts[symb][0] >= 1 and time_tick < time_last_alert_algo_1[symb][0] + 2.1 * 60 * 60 * 1000:
            try:
                ...
                # pass
                bot.send_message(error_tg, f'ver 31 kline_MT(0,k) fail {symb}\n\n{e}')
        ...
        if alerts[symb][1] >= 1 and time_tick < time_last_alert_algo_1[symb][1] + 2.1 * 60 * 60 * 1000:
            try:
                ...
                # pass
                bot.send_message(error_tg, f'ver 31 kline_MT(1,k) fail {symb}\n\n{e}')
...
def rewrite(symb, price, time, channel_data_price, price_acc, high_acc, low_acc):
    ...
    try:
        ...
        bot.send_message(error_tg, f'rewrite 31\n\n{e}')
    


def save():
    try:
        with open('/projects/Ver31/channel_price.txt', 'w') as file:
            file.write(json.dumps(channel_price))
                
        with open('/projects/Ver31/channel_time.txt', 'w') as file:
            file.write(json.dumps(channel_time))

        with open('/projects/Ver31/alerts_MT.txt', 'w') as file:
            file.write(json.dumps(alerts_MT)) 

        with open('/projects/Ver31/channel_cond.txt', 'w') as file:
            file.write(json.dumps(channel_cond))   

        with open(f'/projects/Ver31/stats/{str(day_check)}_stat.txt', 'w') as file:
            file.write(json.dumps(alerts_stat))

        with open('/projects/Ver31/stats/alerts_stat.txt', 'w') as file:
            file.write(json.dumps(alerts_stat))
            
        with open('/projects/Ver31/stats/day_check.txt', 'w') as file:
            file.write(json.dumps(day_check))

        with open('/projects/Ver31/stats_4h/alerts_stat.txt', 'w') as file:
            file.write(json.dumps(alerts_stat_4h))

        with open('/projects/Ver31/stats_4h/alerts_stat_1h.txt', 'w') as file:
            file.write(json.dumps(alerts_stat_1h))
            
        with open('/projects/Ver31/stats_4h/hour_check_1h.txt', 'w') as file:
            file.write(json.dumps(hour_check_1h))
        
        with open('/projects/Ver31/sl_time.txt', 'w') as file:
            file.write(json.dumps(sl_time))

        with open('/projects/Ver31/rsl_time.txt', 'w') as file:
        ...
        with open('/projects/Ver31/high_ser.txt', 'w') as file:
        ...
        with open('/projects/Ver31/low_ser.txt', 'w') as file:
        ...
        with open('/projects/Ver31/close_ser.txt', 'w') as file:
            file.write(json.dumps(close_ser_dict))
        del close_ser_dict 

        with open('/projects/Ver31/lim_amp.txt', 'w') as file:
            file.write(json.dumps(lim_amp))

        with open('/projects/Ver31/lim_atr.txt', 'w') as file:
            file.write(json.dumps(lim_atr))
        '''
        with open('/projects/Ver31/f1.txt', 'w') as file:
            file.write(json.dumps(f1))

        with open('/projects/Ver31/f2.txt', 'w') as file:
            file.write(json.dumps(f2))
        '''
        with open('/projects/Ver31/current_minute_start.txt', 'w') as file:
            file.write(json.dumps(current_minute_start))

        with open('/projects/Ver31/current_minute_prices.txt', 'w') as file:
            file.write(json.dumps(current_minute_prices))

        with open('/projects/Ver31/count_trades.txt', 'w') as file:
            file.write(json.dumps(count_trades))

        with open('/projects/Ver31/time_last_kline.txt', 'w') as file:
            file.write(json.dumps(time_last_kline))

        with open('/projects/Ver31/red_stop.txt', 'w') as file:
            file.write(json.dumps(red_stop))

        with open('/projects/Ver31/orange_stop.txt', 'w') as file:
            file.write(json.dumps(orange_stop))
        
        with open('/projects/Ver31/yellow_stop.txt', 'w') as file:
            file.write(json.dumps(yellow_stop))

        with open('/projects/Ver31/last_atr.txt', 'w') as file:
            file.write(json.dumps(last_atr))

        with open('/projects/Ver31/orange_ten_min_start.txt', 'w') as file:
            file.write(json.dumps(orange_ten_min_start))

        with open('/projects/Ver31/ten_min_start.txt', 'w') as file:
            file.write(json.dumps(ten_min_start))

        with open('/projects/Ver31/max_average_bar_price.txt', 'w') as file:
            file.write(json.dumps(max_average_bar_price))
        
        with open('/projects/Ver31/green_mile.txt', 'w') as file:
            file.write(json.dumps(green_mile))
        with open('/projects/Ver31/green_mile_price.txt', 'w') as file:
            file.write(json.dumps(green_mile_price))
        with open('/projects/Ver31/green_mile_time.txt', 'w') as file:
            file.write(json.dumps(green_mile_time))
        with open('/projects/Ver31/on_w4.txt', 'w') as file:
            file.write(json.dumps(on_w4))
        with open('/projects/Ver31/on_w4_price.txt', 'w') as file:
            file.write(json.dumps(on_w4_price))
        with open('/projects/Ver31/on_w4_time.txt', 'w') as file:
            file.write(json.dumps(on_w4_time))
        with open('/projects/Ver31/max_price.txt', 'w') as file:
            file.write(json.dumps(max_price))
        with open('/projects/Ver31/stop.txt', 'w') as file:
...
def screen(symb, i, df, time_last_alert_algo_1, price_last_alert_algo_1, df_vol, df_btc, long_short_real, w30_score_real, w5_score_real, volume_real, volume_05_real_2):
        ...
        try:
            try:
                ...
                bot.send_message(error_tg, f'ver 31 df generation failed  in screen({symb}, {i})\n\n{e}')  ### отладка - поиск min([])   -nk-
        ...
        ######################### Конец вставки "Для отрисовки стоп-лоссов" -nk 
            ...
            for k in range(len(channel_time[symb][i])):
                if k % 2 == 0:
                    ...
                    try:
                        ...
                        bot.send_message(error_tg, f'ver 31 channel_cond[symb][i][k] reading failed  in screen({symb}, {i}, {k}),\n\n{e}')  ### отладка - поиск min([])   -nk-
            ...
            try:
                ...
                pts, (tot_r, t33_r, sh_r, low_r, sh2_r, s2l) = afrd.realis_calc_2(hi=hi, lo=lo, alert_price=price_last_alert_algo_1[i])
                af_mark, af_comment = afrd.af_realis_detect_2((tot_r, t33_r, sh_r, low_r, sh2_r, s2l), afrd.LIMITS_SHL)
                sh2_r = sh2_r if sh2_r else ""
                g_string = f"Sh/Lng  {sh_r}  {low_r:+.1f}  {sh2_r} =>  {tot_r}  ({t33_r})"
                
                avg1, avg2, v2_v1 = afrd.vol_features_calc_2(df, time_alert)
                vol_mark, vol_comment = afrd.vol_marc_detect_2((avg1, avg2, v2_v1), afrd.LIMITS_VOL)
                v_string = f"Vol1/avg={avg1}, vol2/avg={avg2}, vol2/vol1={v2_v1}"
                
                first_time_trade = afrd.get_05_time(data_in=df, alert_time=time_alert, alert_price=price_last_alert_algo_1[i])
                features05_tpl = afrd.vol_05_calc_2(df, time_alert, first_time_trade)         ### добавил блок объёмов с 0.5
                v05_mark, v05_comment = afrd.vol_05_marc_detect_2(features05_tpl, afrd.LIMITS_V05)
                ind05 = int(features05_tpl[-1])
            except Exception as e:
                bot.send_message(error_tg, f'ver 31 AF features calc failed  in screen({symb}, {i}),\n\n{e}')  ### отладка - поиск min([])   -nk-
            ...
            df['Low'] = df['Low'].astype(float)
            if len(df['Low']) == 0:
                bot.send_message(error_tg, f'ver 31 len(df["Low"]) == 0 in screen({symb}, {i})')  ### отладка - поиск min([])   -nk-
            ...
            if len(df['Low'][(ind[0]+1):ind_2_line]) == 0:
                bot.send_message(error_tg, f'ver 31 len(df["Low"][(ind[0]+1):ind_2_line]) == 0 in screen({symb}, {i})')  ### отладка - поиск min([])   -nk-
            ...
            if len(df['Close'][:ind[0]]) == 0:
                bot.send_message(error_tg, f"ver 31 len(df['Close'][:ind[0]]) == 0 in screen({symb}, {i})")  ### отладка - поиск min([])   -nk-
            ...
            try:
                df_chanels['w30'] = df_chanels[df_chanels['Time'] <= time_alert_msk -2*60*1000].High.rolling(window = 30).apply(afrd.chanel_widths, args=(df_chanels,))
                df_chanels['w5'] = df_chanels[(df_chanels['Time'] >= time_alert_msk - 60*60*1000)&(df_chanels['Time'] <= time_alert_msk -2*60*1000)].High.rolling(window = 5).apply(afrd.chanel_widths, args=(df_chanels,))
                ...
                w30_score = afrd.w30_scoring_2(w30_50a)
                w5_score = afrd.w5_scoring_2(mean_5)
            except Exception as e:
                bot.send_message(error_tg, f'ver 31 W,w calc failed  in screen({symb}, {i}),\n\n{e}')  ### отладка - поиск min([])   -nk-
            ##################################################################################################
            ...
            chanels_info = [afrd.calculate_chanel(data_alert, candles) for candles in time_before_alert]
            points = []
            ...
            cap = f'ver31 {description} {symb}\n{power_emoji}\nclear_pnl: {MT_pnl_clear}%\ntake_clear_potential: {round(MT_pnl_clear/percent_high*100,2)}%'
            
            title_mess = f'\n\n\nver31 {symb}, price: {price_last_alert_algo_1[i]}, time: {time_pump}\nmax={percent_high}%, min={percent_low}%, coef={count_plus_pnl}/{count_minus_pnl}, Ind_Vol= {index_vol}, {v_string}\ntotal_pnl: {MT_pnl_total}%, clear_pnl: {MT_pnl_clear}%, count_trades: {len(MT_pnl)}, comm + sq: {round(len(MT_pnl)*0.16, 2)}%, take_potential: {round(MT_pnl_total/percent_high*100,2)}%,take_clear_potential: {round(MT_pnl_clear/percent_high*100,2)}%,\npnl_trades: {MT_pnl}, min_pcnt_before: {round(min_pcnt_befor, 2)}, predict_volume: {volume_real}, 05_volume: {volume_05_real_2}, w30_score: {w30_score_real}, w5_score: {w5_score_real}, LONG/SHORT: {long_short_real}' #, PnL% = {percent_order} corr: {round(corr,2)}, corr_before: {round(corr_before,2)}, corr_before_15: {round(corr_before_15,2)}, chanel_30: {narrow_30_chanel}, chanel_5: {narrow_5_chanel},
            ### сохраняем последний трейд для отправки в wtp all
            trade_str = f'ver31: {MT_pnl_clear}% / {round(MT_pnl_clear/percent_high*100,2)}%'
...
############################# Конец вставки "Oтрисовка стоп-лоссов" -nk 
    ...
    #             apds.extend([rsl_apd])
                ...
                bot.send_message(error_tg, f"ver 31 sl drowing failed  in screen({symb}, {i})\n\n{e}")  ### отладка - поиск min([])   -nk-
############################# Конец вставки "Oтрисовка стоп-лоссов" -nk 
            ...
            try:
                ...
                bot.send_message(error_tg, f"ver 31 SL-real - alines failed  in screen({symb}, {i})\n\n{e}")  ### отладка - поиск min([])   -nk-
            ...
            try:
                try:
                    ...
                    ind_limit_ser = pd.Series(afrd.WEIGHTS_2["Total"], index=df.index)
                    ...
                    const_score = af_mark * afrd.WEIGHTS_2["R"] + w30_score * afrd.WEIGHTS_2["W"] + w5_score * afrd.WEIGHTS_2["w"] + 20 * min(0, af_mark - afrd.WEIGHTS_2["real_lim"]) + 20 * min(0, w30_score - afrd.WEIGHTS_2["w30_lim"]) + 10 * min(0, w5_score * afrd.WEIGHTS_2["w5_lim"])
                except Exception as e:
                    bot.send_message(error_tg, f"ver 31 induk failed-1  in screen({symb}, {i})\n\n{e}")  ### отладка - поиск min([])   -nk-
                    features05_tpl_cur = afrd.vol_05_calc_2(df, time_alert)
                    v05_string = "\n".join(map(lambda x: f"{x:.2f}", features05_tpl_cur))
                    v05_mark, v05_comment = afrd.vol_05_marc_detect_2(features05_tpl_cur, afrd.LIMITS_V05)
                try:
                    # print("len(time_ser)=", len(time_ser), )
                    df["Time"] = time_ser[:len(df)].values
                    # print("df.Time.min()=", df.Time.min(), "df.Time.max()", df.Time.max(), "time_alert=", time_alert)
                    if ind05 >= df.index[ind[0]]:
                        # print(".", end="")
                        induk_ser[(df.index >= df.index[ind[0] - 1]) & (df.index < ind05)] = const_score + vol_mark * (afrd.WEIGHTS_2["V"] + afrd.WEIGHTS_2["05"]) + 20 * min(0, vol_mark - afrd.WEIGHTS_2["vol_lim"])
                        for j in range(ind[0], ind[0] + 120):  #                      = 0
                            features05_tpl_cur = afrd.vol_05_calc_2(df, time_alert, df.loc[df.index[j], "Time"])  ### динамичные объёмы аля с 0.5
                            # features05_tpl_cur = afrd.vol_05_calc_2(df, time_alert)  ### динамичные объёмы аля с 0.5
                            v05_current_mark, _ = afrd.vol_05_marc_detect_2(features05_tpl_cur, afrd.LIMITS_V05)
                            induk_ser[df.index[j]] = const_score + v05_current_mark * (afrd.WEIGHTS_2["V"] + afrd.WEIGHTS_2["05"]) + 20 * min(0, vol_mark - afrd.WEIGHTS_2["vol_lim"]) + 20 * min(0, v05_current_mark - afrd.WEIGHTS_2["v05_lim"])
                except Exception as e:
                    bot.send_message(error_tg, f"ver 31 induk failed-2  in screen({symb}, {i})\n\n{e}")  ### отладка - поиск min([])   -nk-
                    features05_tpl_cur = afrd.vol_05_calc_2(df, time_alert)
                    v05_string = "\n".join(map(lambda x: f"{x:.2f}", features05_tpl_cur))
                    v05_mark, v05_comment = afrd.vol_05_marc_detect_2(features05_tpl_cur, afrd.LIMITS_V05)
                try:
                    ind_ampl = (df.High.max() - df.Low.min()) / max(induk_ser.max(), afrd.WEIGHTS_2["Total"]) / 5  # 5 - какую часть графика занять
                    ...
                    induk_real_ser = induk_ser.copy()
                    induk_real_ser[df.index < ind05] = None
                    dont_calc_ser = induk_real_ser[induk_real_ser > ind_limit_ser.iloc[1]]
                except Exception as e:
                    bot.send_message(error_tg, f"ver 31 induk failed-3  in screen({symb}, {i})\n\n{e}")  ### отладка - поиск min([])   -nk-
                    features05_tpl_cur = afrd.vol_05_calc_2(df, time_alert)
                    v05_string = "\n".join(map(lambda x: f"{x:.2f}", features05_tpl_cur))
                    v05_mark, v05_comment = afrd.vol_05_marc_detect_2(features05_tpl_cur, afrd.LIMITS_V05)
                try:                    
                    if len(dont_calc_ser):
                        start_index = dont_calc_ser.index.tolist()[0]
                        try: 
                            start_index = start_index + 0
                        except Exception as e:
                            #################################################################################  ### Отладка -nk-
                            try:
                                    with open('/projects/data/CHECK_DF6.txt', 'a') as f:
                                        f.write(f'\n\ndont_calc_ser.index.tolist()= {dont_calc_ser.index.tolist()}\n\n\n\nstart_index= {start_index} ')
                            except Exception as e:
                                bot.send_message(error_tg, f'CHECK_DF6 UPD symb={symb}, time_alert={time_alert}\n\n{e}')
                            #################################################################################  ### Отладка -nk-               
                        induk_real_ser[induk_real_ser.index > start_index] = None
                        features05_tpl_cur = afrd.vol_05_calc_2(df, time_alert, df.loc[start_index, "Time"])
                        v05_string = "\n".join(map(lambda x: f"{x:.2f}", features05_tpl_cur))
                        v05_mark, v05_comment = afrd.vol_05_marc_detect_2(features05_tpl_cur, afrd.LIMITS_V05)
                    else:
                        v05_mark, v05_comment = afrd.vol_05_marc_detect_2(features05_tpl, afrd.LIMITS_V05)
                    ...
                    bot.send_message(error_tg, f"ver 31 induk failed-4  in screen({symb}, {i})\n\n{e}")  ### отладка - поиск min([])   -nk-
                    features05_tpl_cur = afrd.vol_05_calc_2(df, time_alert)
                    v05_string = "\n".join(map(lambda x: f"{x:.2f}", features05_tpl_cur))
                    v05_mark, v05_comment = afrd.vol_05_marc_detect_2(features05_tpl_cur, afrd.LIMITS_V05)
                try:
                    if first_time_trade == 0:
                        v05_comment, v05_string = "нет 0.5", "нет 0.5"
                    total_score = (vol_mark * afrd.WEIGHTS_2["V"] + v05_mark * afrd.WEIGHTS_2["05"] + af_mark * afrd.WEIGHTS_2["R"] + w30_score * afrd.WEIGHTS_2["W"] + w5_score * afrd.WEIGHTS_2["w"])
                    ...
                    apds.extend([mpf.make_addplot(induk_ser, secondary_y=False, color="b", linestyle="-", linewidths=0.03, alpha=0.7),
                                ...
                                mpf.make_addplot(induk_real_ser, secondary_y=False, color="darkred", linestyle="-", linewidths=0.03, alpha=1)
                                ])
                except Exception as e:
                    bot.send_message(error_tg, f"ver 31 induk failed-1  in screen({symb}, {i})\n\n{e}")  ### отладка - поиск min([])   -nk-
                    features05_tpl_cur = afrd.vol_05_calc_2(df, time_alert)
                ...
                bot.send_message(error_tg, f"ver 31 induk failed  in screen({symb}, {i})\n\n{e}")  ### отладка - поиск min([])   -nk-
                features05_tpl_cur = afrd.vol_05_calc_2(df, time_alert)
                v05_string = "\n".join(map(lambda x: f"{x:.2f}", features05_tpl_cur))
                v05_mark, v05_comment = afrd.vol_05_marc_detect_2(features05_tpl_cur, afrd.LIMITS_V05)
            ################# Конец вставки индюк ###############            
            ...
            if percent_high >= 12 or percent_low <= -12:
                ...
                fig.savefig(fname=buf6,dpi=100,pad_inches=0.25)   ### Лучше закоментить потом  -nk-
            ...
            try:
                ...
                combo_formula_string = f'{afrd.WEIGHTS_2["vol_lim"]}{afrd.WEIGHTS_2["v05_lim"]}{afrd.WEIGHTS_2["real_lim"]}{afrd.WEIGHTS_2["w30_lim"]}{afrd.WEIGHTS_2["w5_lim"]}  {afrd.WEIGHTS_2["V"]} {afrd.WEIGHTS_2["05"]} {afrd.WEIGHTS_2["R"]} {afrd.WEIGHTS_2["W"]} {afrd.WEIGHTS_2["w"]}  ({afrd.WEIGHTS_2["Total"]} Б)'
                ...
                if (total_score > afrd.WEIGHTS_2["Total"] and vol_mark >= afrd.WEIGHTS_2["vol_lim"] and \
                    v05_mark >= afrd.WEIGHTS_2["v05_lim"] and af_mark >= afrd.WEIGHTS_2["real_lim"] and \
                    w30_score >= afrd.WEIGHTS_2["w30_lim"] and w5_score >= afrd.WEIGHTS_2["w5_lim"]):
                ...
                bot.send_message(error_tg, f"ver 31 af-feaures drawing failed  in screen({symb}, {i})\n\n{e}")  ### отладка - поиск min([])   -nk-
            ...
            #if percent_high >= 3:
                #bot.send_photo(wtp_screen_tg, open(f'/projects/Ver31/mt_screen/screen{symb}.jpeg','rb'), caption=cap)
            ...
            return f_id, trade_str
                ...
                bot.send_message(error_tg, f'a1_v31 fail screen {symb} i={i}\n\n{e}')
